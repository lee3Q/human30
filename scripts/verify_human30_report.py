...
from scripts.verify_human30_repro import require, verify_axis
...
def verify_verdict(verdict: dict, root: Path = ROOT) -> None:
    ...
    require(type(verdict) is dict and set(verdict) == {"schema_version", "claims", "evidence_sha256"}, "verdict schema mismatch")
    require(verdict["schema_version"] == 1, "verdict schema version mismatch")
    claims = verdict["claims"]
    require(type(claims) is dict and set(claims) == set(CLAIMS), "unsupported or missing claim")
    for key, (status, scope) in CLAIMS.items():
        require(claims[key] == {"status": status, "scope": scope}, f"unsupported claim: {key}")

    expected_paths = {path.as_posix() for path in evidence_files()}
    hashes = verdict["evidence_sha256"]
    require(type(hashes) is dict and set(hashes) == expected_paths, "evidence path mismatch")
    for axis in AXES:
        verify_axis(root / EVIDENCE_ROOT, axis)
    for path in evidence_files():
        actual = hashlib.sha256((root / path).read_bytes()).hexdigest()
        require(hashes[path.as_posix()] == actual, f"evidence hash mismatch: {path}")
